...
// ========================================================================
// SHIFT + CLICK – dodawanie elementów do selekcji
// ========================================================================

let selectedNodes = [];

function setupShiftClickSelection(page) {
    ...
    stage.on("click", (e) => {
        ...
        // kliknięcie w tło → odznacz wszystko
        if (target === stage) {
            selectedNodes = [];
        ...
        // tylko elementy, które można zaznaczać
        const isSelectable =
            target.hasName("text-editable") ||
            target.hasName("draggable-image") ||
            target.hasName("box-editable") ||
            target.hasName("barcode-image");

        if (!isSelectable) return;

        // SHIFT → dokładamy element do selekcji
        if (e.evt.shiftKey) {
            if (!selectedNodes.includes(target)) {
                selectedNodes.push(target);
            }
        } 
        // BEZ SHIFT → pojedynczy wybór
        else {
            selectedNodes = [target];
        }

        // usuń poprzednie transformatory
        layer.find("Transformer").forEach(t => t.destroy());

        // grupa lub pojedynczy
        if (selectedNodes.length > 1) {
            enableGroupTransform(selectedNodes, page);
        } else {
            enableTransform(selectedNodes[0], page);
        }
