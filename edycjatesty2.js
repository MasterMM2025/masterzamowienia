...
const DEFAULT_STYLES = {
  ...
  // NOWE DOMYŚLNE USTAWIENIA DLA PUDEŁEK
  boxStyle: { 
    strokeWidth: 1, 
    strokeColor: '#000000', 
    fill: '#ffffff', 
...
function createPageEditPanel() {
  ...
  <label>Grubość ramki:
    <input type="number" id="boxStrokeWidth" min="0" max="20" value="1">
  </label>

  <label>Kolor ramki:
    <input type="color" id="boxStrokeColor" value="#000000">
  </label>

  <label>Kolor wypełnienia:
    <input type="color" id="boxFillColor" value="#ffffff">
  </label>

  <label>Zaokrąglenie rogów:
    <input type="number" id="boxCornerRadius" min="0" max="50" value="0">
  </label>

  <button class="reset-btn" id="resetBox">Przywróć domyślne</button>
</div>

  `;

  document.body.appendChild(pageEditPanel);
    // 🔥 Użytkownik dotknął ustawień pudełek – zaznacz flagę
  const boxInputs = [
    'boxStrokeWidth',
    'boxStrokeColor',
    'boxFillColor',
  ...
  boxInputs.forEach(id => {
    const el = document.getElementById(id);
    if (el) {
      el.addEventListener('input', () => {
        userChangedBox = true;   // od tej chwili pudełka wolno nadpisać
      });
    }
  });

  // Przycisk "Przywróć domyślne" dla pudełek
  const resetBoxBtn = document.getElementById('resetBox');
  if (resetBoxBtn) {
    resetBoxBtn.addEventListener('click', () => {
      const def = DEFAULT_STYLES.boxStyle;
      document.getElementById('boxStrokeWidth').value  = def.strokeWidth;
      document.getElementById('boxStrokeColor').value  = def.strokeColor;
      document.getElementById('boxFillColor').value    = def.fill;
      document.getElementById('boxCornerRadius').value = def.cornerRadius;
      userChangedBox = true; // też traktujemy jako zmianę
    });
  }
...
window.openPageEdit = function(page) {
  ...
  document.getElementById('applyPageEditBtn').onclick = () => {
    ...
    const newStyles = {
      ...
      // ➕ NOWY STYL DLA PUDEŁEK (BOXÓW)
boxStyle: {
  strokeWidth: parseFloat(document.getElementById('boxStrokeWidth').value),
  strokeColor: document.getElementById('boxStrokeColor').value,
  fill: document.getElementById('boxFillColor').value,
  cornerRadius: parseFloat(document.getElementById('boxCornerRadius').value)
}
};
...
targetPages.forEach(p => {
  ...
  p.layer.getChildren().forEach(obj => {
    ...
    // === STYLE DLA PUDEŁEK ===
    if (obj.getAttr('isBox') === true) {

      if (userChangedBox) { // 🔥 zmieniamy TYLKO gdy użytkownik edytował ramki
        const s = p.settings.boxStyle;

        const newStroke      = s.strokeColor || obj.stroke() || '#cccccc';
        const newStrokeWidth = isNaN(s.strokeWidth)  ? obj.strokeWidth() : s.strokeWidth;
        const newFill        = s.fill || obj.fill() || '#ffffff';
        const newRadius      = isNaN(s.cornerRadius) ? obj.cornerRadius() : s.cornerRadius;

        obj.stroke(newStroke);
        obj.strokeWidth(newStrokeWidth);
        obj.cornerRadius(newRadius);

        // 🛑 Wypełnienie NIE zostanie zmienione na czarne — zabezpieczenie
        if (!newFill || newFill === "#000000") {
          obj.fill("#ffffff");
        } else {
          obj.fill(newFill);
        }
      ...
      return; // kończymy tylko stylizację pojedynczego pudełka ✔️
    }



    // === TEKSTY — pozostały kod bez zmian ===
    ...
    if (isName) applyStyle(obj, p.settings.nameStyle);
    if (isIndex) applyStyle(obj, p.settings.indexStyle);
